...
const { sendMail } = require("../mailer"); // ✅ Use sendMail from your Brevo mailer.js

/* ─────────────────────────────────────────────────────────────
   Config & helpers
   ───────────────────────────────────────────────────────────── */
const OTP_TTL_MIN = Number(process.env.OTP_TTL_MIN || 10); // minutes
const MAX_ATTEMPTS = 5;          // wrong OTP attempts before wipe
const COOLDOWN_MS = 10 * 1000;   // 10s gap between requests for same email+purpose
...
function buildOtpEmail({ purpose, otp }) {
  ...
  const title =
    purpose === "register" ? "Email Verification OTP" : "Password Reset OTP";
...
exports.sendOtp = async (req, res) => {
  try {
    ...
    // Anti-spam cooldown
    const lastAny = await Otp.findOne({ email: emailLower, purpose }).sort({
      createdAt: -1,
    });
    if (
      lastAny &&
      lastAny.createdAt &&
      Date.now() - lastAny.createdAt.getTime() < COOLDOWN_MS
    ) {
      const wait = Math.ceil(
        (COOLDOWN_MS - (Date.now() - lastAny.createdAt.getTime())) / 1000
      );
    ...
    // Reuse existing OTP if unexpired
    ...
    console.log(`[MAIL] OTP sent successfully to ${emailLower}`);
    return res.json({ success: true, message: "OTP sent successfully" });
  } catch (err) {
    console.error("OTP send error:", err.message);
    const msg = String(
      err?.response?.data?.errors?.[0]?.message || err?.message || err
    );
    ...
    if (/535|Invalid login|Authentication failed/i.test(msg)) {
      return res.status(503).json({
        success: false,
        message:
          "Email service authentication failed. Please check Brevo SMTP_USER and SMTP_PASS in .env.",
      });
    }

    if (/rate|quota|limit/i.test(msg)) {
      return res.status(503).json({
        success: false,
        message:
          "Email sending limit reached. Please try again later or upgrade your Brevo plan.",
      });
    }

    return res
      .status(500)
      .json({ success: false, message: "Internal Server Error: " + msg });
  }
};
...
exports.resetPasswordWithOtp = async (req, res) => {
  try {
    ...
    if (!email || !otp || !newPassword)
      return res
        .status(400)
        .json({ success: false, message: "Email, OTP and new password required" });
